...
namespace my_drake_examples
{
  namespace biped
  {
    ...
    template <typename T>
    class WholeBodyController final : public drake::systems::LeafSystem<T>
    {
    public:
      DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(WholeBodyController)

      WholeBodyController(double time_step, double offset,
      ...
      const drake::systems::InputPort<T> &get_stance_input_port() const
      {
        return *stance_input_port_;
      }
    ...
    private:
      ...
      const drake::systems::InputPort<T> *stance_input_port_{nullptr};
