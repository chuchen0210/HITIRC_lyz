...
namespace my_drake_examples
{
  namespace biped
  {
    ...
    template <typename T>
    TauController<T>::TauController(double time_step, double offset,
        ...
        : time_step_(time_step), plant_(plant)
    {
      ...
      this->DeclarePeriodicDiscreteUpdateEvent(
          time_step, offset, &TauController::Update);
    }

    template <typename T>
    void TauController<T>::Update(const Context<T> &context,
                                  DiscreteValues<T> *next_state) const
    {
      ...
      const auto &left_stance = get_left_stance_input_port().Eval(context)(0);
      const auto &right_stance = get_right_stance_input_port().Eval(context)(0);
