...
class TaskSolver(TaskSolverBase):
    ...
    def next_action(self, obs: dict) -> dict:
        ...
        quat = agent["body_state"]["world_orient"]    #获取四元数
        rpy = t3d.euler.quat2euler(quat, axes="sxyz") #四元数姿态转成欧拉角
        #rpy = [roll, pitch, yaw]
        #roll 绕x轴旋转（横滚角）：机器人的翻滚
        #pitch 绕y轴旋转（俯仰角）：机器人的抬头和低头
        #yaw 绕z轴旋转（航向角）：机器人的朝向
        ...
        target_dir = np.arctan2(dy, dx) #单位弧度
