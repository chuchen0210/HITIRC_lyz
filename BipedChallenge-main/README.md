...
1. **Download Docker Image**
   ...
   #断点续传
   wget -c https://roban.lejurobot.com/humanoid-tongverselite/tongverselite-release-docker_20240104.tar.gz -O tongverselite-release-docker.tar.gz
...
class BipedWalkingCtrlClient(PersistentTcpClient):
    def send_request(self, msg):
        ...
        #Python 字典格式的控制请求转成二进制，通过 TCP 发送给机器人控制器，然后接收返回的二进制响应、解		  #码并还原为字典格式
...
As the scene is static（静态）, you can hard-code any necessary information to ensure the robot can accomplish the task from a random initial state.
